class UsersController < ApplicationController
  ...
  def update
    ...
    if @user.authenticate(params[:password_old])
      ...
      flash_now :danger, t(:pwdoesnotmatch) if !params[:user][:password].blank? || params[:user][:email] != @user.email
    end
    @user.reload
    render 'edit'
  end
  ...
  # E-Mail verification
  def activate
    @user = User.find_by!(username: params[:username], update_token: params[:token], activated: false)
    @user.activated = true
    @user.save!
  ...
  def update_email
    @user = User.find_by!(username: params[:username], update_token: params[:token])
