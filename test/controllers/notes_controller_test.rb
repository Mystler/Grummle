...
class NotesControllerTest < ActionController::TestCase
  ...
  test 'creation works' do
    ...
    assert_difference('Note.count') do
      post :create, params: { note: {title: '12345', text: 'Blah'} }
    end
  ...
  test 'update works' do
    do_login
    patch :update, params: { id: notes(:testnote).permalink, note: {title: 'Updated', text: 'Blah'} }
    assert_redirected_to note_path(assigns(:note))
  ...
  test 'markdown displayed' do
    do_login
    get :show, params: { id: notes(:markupnote).permalink }
  ...
  test 'access restricted' do
    do_login
    get :show, params: { id: notes(:userless).permalink }
    assert_redirected_to notes_path, 'Not redirected from userless note'
    assert flash[:danger]
    get :edit, params: { id: notes(:userless).permalink }
    assert_redirected_to notes_path
    assert flash[:danger]
    patch :update, params: { id: notes(:userless).permalink, note: {title: 'Test'} }
    assert_redirected_to notes_path
    assert flash[:danger]
    assert_raises(ActiveRecord::RecordNotFound) do
      delete :destroy, params: { id: notes(:userless).permalink }
    end
    do_logout
  end

  test 'public notes' do
    get :show, params: { id: notes(:publicnote).permalink }
  ...
  test 'edit public notes' do
    do_login
    get :edit, params: { id: notes(:publicnote).permalink }
    assert_redirected_to notes_path
    assert flash[:danger]
    patch :update, params: { id: notes(:publicnote).permalink, note: {title: 'Test'} }
    assert_redirected_to notes_path
    assert flash[:danger]
    do_logout
  end
