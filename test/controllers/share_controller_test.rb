...
class ShareControllerTest < ActionController::TestCase
  test 'sharing works' do
    do_login
    get :index, params: { note_id: notes(:sharednote).permalink }
    assert_response :success
    assert_difference('Share.count') do
      post :create, params: { note_id: notes(:sharednote).permalink, username: users(:sharetest).username }
  ...
  test 'sharing access restricted' do
    do_login
    get :index, params: { note_id: notes(:userless).permalink }
    assert_redirected_to notes_path
    assert flash[:danger]
    post :create, params: { note_id: notes(:userless).permalink, username: users(:testuser).username }
    assert_redirected_to notes_path
    assert flash[:danger]
    delete :create, params: { note_id: notes(:userless).permalink, id: 123 } # ID should not matter for redirect
    assert_redirected_to notes_path
    assert flash[:danger]
    do_logout
  end
